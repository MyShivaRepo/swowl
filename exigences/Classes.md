...
|---|---|
| **Alors** | son `range` est affiché entre parenthèses juste après le nom avec une flèche `→` et l'icône de la cible : un rond marron (●) avec lien navigable (`APP.navigateTo('classes', …)`) pour une `classe` cible, ou un petit marqueur vert pour un type `xsd:` (non navigable). Ce même rendu `(→ …)` — y compris le lien de classe navigable — est utilisé dans **les deux** sections *Inherited* et *Asserted*. |

| **Si** | une `propriété` avec un `range` non vide est affichée dans l'une ou l'autre section, |
|---|---|
| **Alors** | la **multiplicité** est affichée à l'intérieur de la parenthèse du range, juste avant la cible, sous la forme `single` ou `multiple` (ex. `(→ multiple ● Classe)`), déduite de la caractéristique `functional` de la propriété : `functional` ⇒ `single`, sinon `multiple`. |
