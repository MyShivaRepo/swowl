...
**Then** :
- the system builds a minimal property object (`domain: [], range: [], inverseOf: null, characteristics: {}, propertyChainAxiom: []`) and registers it via `API.createOP(prop)` ;
- on success, it sets `_selectedId` and `_editingId`, refreshes the global state via `APP.refresh()`, then re-renders the tab via `APP.renderSection('object-properties')` ;
- on error, it displays the message via `UI.error()`.
...
**Then** :
- the system updates the hidden field `#op-inverse-value` with the selected ID ;
- replaces in `#op-inverse-body` the placeholder "— none —" with an item containing a delete button ;
- hides the `+` button via `display:none` ;
- closes the picker `op-inverse-picker` ;
- triggers `autoSave()`.
...
**and** the inference engine returns `inferred_inverse_properties` results concerning this property,
...
**If** an existing property is being edited (`_editingId !== null`)
**and** a form field triggers an `onchange` event,
...
**Then** :
- the system collects the identifier (`op-id`), annotations, domain, range, super-properties, the inverse property and the 7 characteristics ;
- validates the identifier via `_validateId()` ;
- if the property is new, calls `API.createOP()` ; otherwise calls `API.updateOP(originalId, prop)` ;
- if the identifier has changed, displays a rename message ;
- in all cases, refreshes the tab after success.
...
**Then** :
- the system places as root a fixed node `owl:topObjectProperty` (clickable via `OPEditor.selectTopProp()`) ;
- recursively calls `_renderNode()` for each root property (those without a parent in `subPropertyOf`) ;
- if no property exists, displays the text "No ObjectProperty" instead.
...
**Then** :
- the node is `draggable="true"` and carries the handlers `ondragstart`, `ondragover`, `ondragleave`, `ondrop`, `ondragend` ;
- if the property has a non-null `inverseOf` field, a badge `↔ <id>` is displayed next to the label ;
- indentation is calculated according to depth (`depth * 16 + 6` px) ;
- children are rendered in a container `op-tcn-<id>` whose visibility is controlled by `OPEditor._expanded`.
...
**Then** :
- the system sets `_selectedId = null` and `_topPropSelected = true` ;
- applies the CSS class `selected` to the root node ;
- replaces the content of the detail panel with a welcome message containing a "＋ Create Object Property" button ;
- clears the "Super Properties" panel ;
- updates the state of the toolbar buttons via `_updateTreeButtons()`.
...
**Then** :
- the system updates `_selectedId` and applies the `selected` class to the correct tree item ;
- replaces the detail panel with `renderForm(prop)` ;
- initialises the internal vertical resizers via `_initHResizers` ;
- refreshes the "Super Properties" panel via `_updateSuperPanel()` ;
- updates the buttons via `_updateTreeButtons()` ;
- triggers the loading of inverse inferences via `_loadInferredInverse()`.
...
**Then** :
- the system creates a DOM element `div#op-ctx-menu` positioned at the mouse coordinates ;
- it contains the actions "Add Child Property", "Add Sibling Property" (only if a non-root property is targeted), and "Delete" ;
- the menu closes automatically on the next outside click via a `click` listener in capture mode.
...
**Then** :
- the system fills the panel `#op-sub-list` with the direct super-properties of the selected property ;
- for each direct super-property, it builds the complete ancestor chain up to `owl:topObjectProperty` and displays each link with increasing indentation ;
- direct super-properties have a delete button `✕` ;
- transitive ancestors are displayed in italics with reduced opacity (0.75) ;
- clicking on any ancestor navigates to that property via `APP.navigateTo()`.
...
**Then** :
- the system calls `_initSplitPane()` ;
- if `_topPropSelected` is true, calls `selectTopProp()` again ;
- if an ID is stored in `_selectedId`, calls `selectProp(_selectedId)` again ;
- thereby ensuring that the current selection is maintained after each re-render.
