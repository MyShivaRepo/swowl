...
| **If** | the `ontology` contains `object `ObjectProperties` organised in a hierarchy, |
|---|---|
| **Then** | the `ObjectProperty` tree faithfully reflects the specialisation relationships between `ObjectProperties`, distinguishing root `ObjectProperties` from those that depend on them, and presenting them in alphabetical order. |
...
| **If** | the ontologist selects or creates a `ObjectProperty` in the tree, |
|---|---|
| **Then** | the complete path from the root to that `ObjectProperty` is automatically expanded, without the user having to manually open each intermediate level. |
...
| **If** | the ontologist wants to specialise an existing `ObjectProperty` by creating a subproperty directly positioned beneath it, |
|---|---|
| **Then** | the new `ObjectProperty` is created as a child of the selected `ObjectProperty`, and the parent `ObjectProperty` is automatically expanded in the tree to make the new entry visible. |
...
| **If** | the ontologist wants to create a new `ObjectProperty` at the same hierarchical level as an existing `ObjectProperty`, |
|---|---|
| **Then** | the new `ObjectProperty` is positioned under the same parents as the reference `ObjectProperty`, without requiring any additional configuration of hierarchical relationships. |
...
| **If** | the ontologist creates a new `object `ObjectProperty`, |
|---|---|
| **Then** | a unique identifier is automatically assigned to it, with no risk of collision with existing `ObjectProperties` in the `ontology`. |
...
| **If** | the ontologist confirms the creation of a new `object `ObjectProperty`, |
|---|---|
| **Then** | the `ObjectProperty` is saved in the `ontology` with its hierarchical relationships, and the tree is updated to immediately reflect this new entry. |
...
| **If** | the ontologist moves a `ObjectProperty` in the tree by drag-and-drop to change its hierarchical position, |
|---|---|
| **Then** | the `ObjectProperty` is attached to its new parent (or placed at the root if dropped onto the root node), and the `ontology` is updated accordingly. |
...
| **If** | the ontologist attempts to move a `ObjectProperty` onto one of its own descendants in the tree, |
|---|---|
...
| **If** | the ontologist wishes to delete an `object `ObjectProperty` from the `ontology`, |
|---|---|
...
| **If** | the ontologist adds a class to the domain of an `object `ObjectProperty`, |
|---|---|
| **Then** | the class is associated with the `ObjectProperty` as a domain constraint, and the `ObjectProperty` is automatically saved; if no domain is defined, `owl:Thing` is implicitly indicated. |

| **If** | the ontologist removes a class from the domain of an `object `ObjectProperty`, |
|---|---|
| **Then** | the domain constraint is deleted, the `ObjectProperty` is automatically saved, and `owl:Thing` is displayed again if the domain becomes empty. |
...
| **If** | the ontologist adds a class to the range of an `object `ObjectProperty`, |
|---|---|
| **Then** | the class is associated with the `ObjectProperty` as a range constraint, and the `ObjectProperty` is automatically saved; if no range is defined, `owl:Thing` is implicitly indicated. |

| **If** | the ontologist removes a class from the range of an `object `ObjectProperty`, |
|---|---|
| **Then** | the range constraint is deleted, the `ObjectProperty` is automatically saved, and `owl:Thing` is displayed again if the range becomes empty. |
...
| **If** | the ontologist associates an inverse `ObjectProperty` with the `ObjectProperty` being edited, |
|---|---|
| **Then** | the inverse relationship is saved and the `ObjectProperty` is automatically saved. |
...
| **If** | an inverse `ObjectProperty` is already defined for the `ObjectProperty` being edited, |
|---|---|
| **Then** | it is not possible to add a second one; furthermore, `ObjectProperties` that already have an inverse defined (pointing to another `ObjectProperty`) are not offered in the selection list. |
...
| **If** | the ontologist wishes to remove the inverse relationship from a `ObjectProperty`, |
|---|---|
| **Then** | the inverse `ObjectProperty` is dissociated and the ability to define a new one is immediately restored. |
...
| **If** | a `ObjectProperty` is selected and the reasoner detects implicit inverse relationships involving it, |
|---|---|
...
| **If** | the ontologist attaches an `object `ObjectProperty` to a super-`ObjectProperty`, |
|---|---|
| **Then** | the hierarchical relationship is saved and the `ObjectProperty` is automatically saved. |

| **If** | the ontologist removes the attachment to a super-`ObjectProperty`, |
|---|---|
| **Then** | the hierarchical relationship is deleted and the `ObjectProperty` is automatically saved. |
...
| **If** | the ontologist `views` the form of an `object `ObjectProperty`, |
|---|---|
...
| **If** | the ontologist saves the `ObjectProperty`, |
|---|---|
| **Then** | the state of each characteristic is included in the persisted data of the `ObjectProperty`. |
...
| **If** | the ontologist modifies a field in the form of an already existing `ObjectProperty`, |
|---|---|
...
| **If** | the ontologist saves an `object `ObjectProperty` (new or existing), |
|---|---|
| **Then** | all entered information is persisted in the `ontology` — identifier, annotations, domain, range, super-`ObjectProperties`, inverse `ObjectProperty` and characteristics — and the tree is updated to reflect the changes, including any renaming. |
...
| **If** | the ontologist wishes to create an `object `ObjectProperty` directly from a class record, using that class as domain, |
|---|---|
| **Then** | a new `object `ObjectProperty` is created with the current class as domain, and the application automatically navigates to the `object `ObjectProperties` tab to allow further editing. |
...
| **If** | the ontologist opens the `object `ObjectProperties` tab, |
|---|---|
| **Then** | the tree presents first the root `ObjectProperty` `owl:topObjectProperty`, followed by all `ontology` `ObjectProperties` organised according to their hierarchical relationships; if the `ontology` contains no `object `ObjectProperty`, a message explicitly indicates this. |
...
| **If** | a node in the `object `ObjectProperty` tree is displayed, |
|---|---|
| **Then** | if the `ObjectProperty` has a declared inverse `ObjectProperty`, it is visually indicated alongside the `ObjectProperty` name; indentation reflects the hierarchical level; child `ObjectProperties` are shown or hidden according to the node's expansion state; the node can be moved by drag-and-drop. |
...
|---|---|
| **Then** | the detail panel offers to create a new `object `ObjectProperty`, no existing `ObjectProperty` is selected, and the tree actions are updated accordingly. |
...
| **If** | the ontologist selects a `ObjectProperty` in the tree, |
|---|---|
| **Then** | the edit form for that `ObjectProperty` is displayed in the detail panel, the super-`ObjectProperty` chain is updated, and inferred inverse `ObjectProperties` are loaded. |
...
|---|---|
| **Then** | only the child `ObjectProperty` creation action is available; the sibling `ObjectProperty` creation and deletion actions are disabled. |

| **If** | an ordinary `ObjectProperty` is selected, |
|---|---|
| **Then** | all three actions (create child `ObjectProperty`, create sibling `ObjectProperty`, delete) are available. |

| **If** | no `ObjectProperty` is selected in the tree, |
|---|---|
...
|---|---|
| **Then** | the child `ObjectProperties` of that node are shown or hidden, and the visual indicator reflects the current state (expanded or collapsed). |
...
|---|---|
| **Then** | a context menu is displayed at the cursor coordinates, offering relevant actions according to the targeted node (create child `ObjectProperty`, create sibling `ObjectProperty`, delete); the menu closes automatically as soon as a click is made outside it. |
...
| **If** | the ontologist selects an `object `ObjectProperty` in the tree, |
|---|---|
| **Then** | a complete form is displayed with the `ObjectProperty` identifier, its computed IRI, its annotations, domain and range constraints, the inverse `ObjectProperty`, OWL characteristics, and the `ObjectProperty`'s usages in the `ontology`. |

| **If** | the `ObjectProperty` is new, |
|---|---|
| **Then** | saving is triggered on blur of the identifier field; for an existing `ObjectProperty`, it is triggered on each field modification. |
...
| **If** | the ontologist adds a `label` or `comment` type annotation to the `ObjectProperty`, |
|---|---|
| **Then** | a new annotation row is added in the Annotations section of the form. |

| **If** | the ontologist adds an annotation of another type by selecting an `annotation `ObjectProperty`, |
|---|---|
| **Then** | an annotation row of type `other` is created with the specified `ObjectProperty`. |

| **If** | the ontologist deletes an annotation row, |
|---|---|
| **Then** | the row is removed from the form and the `ObjectProperty` is automatically saved if it is in edit mode. |
...
| **If** | the ontologist selects a `ObjectProperty` in the tree, |
|---|---|
| **Then** | the "Super `ObjectProperties`" panel displays the complete hierarchical chain going up to `owl:topObjectProperty`: direct super-`ObjectProperties` are editable (deletable), while transitive ancestors are presented visually as such; clicking on any ancestor navigates to its record. |
...
| **If** | the ontologist consults the `object `ObjectProperties` tab, |
|---|---|
| **Then** | the interface is divided into a left panel displaying the tree and the super-`ObjectProperty` chain, and a right panel displaying the edit form; each panel is resizable by dragging a separator handle. |
...
| **If** | the `object `ObjectProperties` tab is reloaded following an action (creation, modification, deletion), |
|---|---|
| **Then** | the `ObjectProperty` that was selected before the reload is automatically re-selected, preserving the ontologist's working context. |
